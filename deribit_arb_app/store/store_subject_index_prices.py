...
class StoreSubjectIndexPrices(StoreSubjectableInterface):
    ...
    def update_subject(self, index_price: ModelIndexPrice):
        if not index_price.index_name in self.__subject_index_prices:
            self.__subject_index_prices[index_price.index_name] = SubjectIndicator(index_price)
    ...
    def get_subject(self, index: ModelIndex):
        if not index.index_name in self.__subject_index_prices:
            self.__subject_index_prices[index.index_name] = SubjectIndicator(ModelIndexPrice(index_name=index.index_name))
